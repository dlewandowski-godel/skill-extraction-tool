...
public class AnalyticsRepository : IAnalyticsRepository
{
    ...
    public async Task<List<DepartmentSkillsDto>> GetSkillsByDepartmentAsync(CancellationToken ct = default)
    {
        var data = await (
            ...
            join s in _db.Skills on es.SkillId equals s.Id
            where u.Department != null
            select new { u.Department, SkillName = s.Name }
        ).ToListAsync(ct);
        ...
        string? department, CancellationToken ct = default)
    {
        ...
        var coverage = await (
            from es in _db.EmployeeSkills
            join u in _db.Users on es.UserId equals u.Id
            where skillIds.Contains(es.SkillId) && deptNames.Contains(u.Department!)
            group es by new { es.SkillId, u.Department } into g
            select new { g.Key.SkillId, g.Key.Department, Count = g.Count() }
        ).ToListAsync(ct);

        // Total employees per department
        var totals = await _db.Users
            .Where(u => deptNames.Contains(u.Department!))
        ...
        {
            var cov = coverage.FirstOrDefault(c => c.SkillId == req.SkillId && c.Department == req.DepartmentName);
